...
class MessageScreen extends Component<Props> {
  ...
  async componentDidMount() {
    ...
    if ('chatId' in this.props.navigation.state.params) {
      ...
      fetchMessages(this.props.client, chatId, 0).then((data) => {
        this.readMessages(data.messages);
        dispatch(ChatActions.setMessages(data, user.id));
        this.setState({
  ...
  readMessages = (messages) => {
    ...
    const unread = messages.filter((message) =>
      !message.read && message.author.id !== user.id
    );
    if (unread.length > 0) {
      socket.readMessage(unread.map((message) => message.id), partnerId);
    }
  }
